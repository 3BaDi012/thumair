...
export function AdminMessagesPage() {
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    if (!targetUserId.trim() || !message.trim()) {
      setError(bi(locale, 'أدخل معرّف المستخدم والرسالة.', 'Enter a user ID and a message.'));
      return;
    }
    setSending(true);
    try {
  ...
  return (
    <div className="container mx-auto px-6 py-10">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{bi(locale, 'رسائل الإدارة', 'Admin messages')}</h1>
      </div>
      <p className="text-gray-600 dark:text-gray-400 mb-8">
        {bi(locale, 'محادثات الدعم مع المستخدمين.', 'Support conversations with users.')}
      </p>
      ...
      <form onSubmit={(e) => void sendMessage(e)} className="mb-10 space-y-4 max-w-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6">
        <h2 className="font-semibold text-gray-900 dark:text-white">{bi(locale, 'بدء محادثة', 'Start a conversation')}</h2>
        <input
          placeholder={bi(locale, 'معرّف المستخدم (UUID)', 'User ID (UUID)')}
          value={targetUserId}
        ...
        />
        <textarea
          placeholder={bi(locale, 'نص الرسالة الأولى', 'First message')}
          value={message}
        ...
        >
          ...
          {sending ? bi(locale, 'جارٍ الإرسال...', 'Sending...') : bi(locale, 'إرسال وفتح المحادثة', 'Send & open conversation')}
        </button>
      </form>

      <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">{bi(locale, 'المحادثات', 'Conversations')}</h2>
      {loading ? (
        <p className="text-gray-500">{bi(locale, 'جارٍ التحميل...', 'Loading...')}</p>
      ) : convos.length === 0 ? (
        <p className="text-gray-500">{bi(locale, 'لا توجد محادثات بعد.', 'No conversations yet.')}</p>
      ) : (
            ...
            <li key={c.id}>
              ...
              >
                ...
                <p className="text-xs text-gray-500">
                  {new Date(c.created_at).toLocaleString(locale === 'en' ? 'en-US' : 'ar-SA')}
                </p>
              </Link>
            </li>
          ))}
