...
export function AdminFeedbackPage() {
  ...
  return (
    <div className="container mx-auto px-6 py-10">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">الشكاوى والاقتراحات</h1>
      <p className="text-gray-600 dark:text-gray-400 mb-6">قراءات الرضا، الشكاوى، والاقتراحات من المستخدمين.</p>
            ...
            <div key={f.id} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
              <div className="flex items-start justify-between gap-4 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">{new Date(f.created_at).toLocaleString('ar-SA')}</span>
                </div>
                <select
                  value={f.status}
              ...
              {f.email && <p className="text-xs text-gray-500">البريد: {f.email}</p>}
            </div>
          ))}
        </div>
      )}
